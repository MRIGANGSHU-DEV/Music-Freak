...
const HotHitsHindi = ({ setCurrentSong }) => {
  const [songs, setSongs] = useState([]);

  useEffect(() => {
    const fetchSongs = async () => {
      const songsCollection = collection(db, "Hot Hits Hindi");
  ...
  return (
    <div className="px-10 py-8 bg-black text-white">
      <h2 className="text-3xl font-bold">
        Hot Hits <span className="text-pink-500">Hindi</span>
      </h2>
